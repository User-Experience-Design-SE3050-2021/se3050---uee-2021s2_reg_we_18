...
export default function updateCategory({route,navigation}) {
  ...
  useEffect(() => {
    axios.get(`https://riyapola.herokuapp.com/category/${id}`)
    .then((res) => {
      setInputValues({
        type: res.data.type,
        make: res.data.make,
  ...
  const handleSubmit = () => {
    ...
    if (submitForm.mainName == undefined || submitForm.mainName == "") {
      ...
      setBtnDisable(true);
      axios
        .put(`https://riyapola.herokuapp.com/category/${id}`, submitForm)
        .then((res) => {
          if (res.status === 200) {
    ...
    <View style={globalStyles.categoryAddContainer}>
      ...
      >
        ...
        <Picker
          ...
          onValueChange={(itemValue, itemIndex) => setInputValues({...inputValues,type:itemValue})}
